...
void ADC_GPIO_Init(void){ //GPIO初始化设置
    ...
    GPIO_InitStructure.GPIO_Pin = ADC_CH7|ADC_CH3; //选择端口                        
...
void ADC_Configuration(void){ //初始化设置
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_7, 1, ADC_SampleTime_28Cycles5);//ADC1选择信道x,采样顺序y,采样时间n个周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_3, 2, ADC_SampleTime_28Cycles5);//ADC1选择信道x,采样顺序y,采样时间n个周期
